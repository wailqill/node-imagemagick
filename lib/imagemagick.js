var imagemagick = function(opts) {
    var sys, childproc, EventEmitter,
    ...
    for (var p in options)
        if (opts[p] !== undefined) options[p] = opts[p];

    sys = options.sys;
    childproc = options.childproc;
    EventEmitter = options.emitter;
...
readMetadata = function(path, callback) {
  return identify(['-format', '%[EXIF:*]', path], function(err, stdout) {
    var meta = {};
    if (!err) {
