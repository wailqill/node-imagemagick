var imagemagick = function(childproc) {
  ...
  function exec2(file, args /*, options, callback */) {
    ...
    var Wrapper = function(proc) {
      ...
      proc.emitter = proc.emitter || new EventEmitter();
    ...
    if (options.output) {
      child.on('data', options.output.emit.bind(options.output, 'data'));
      child.on('end', options.output.emit.bind(options.output, 'end'));
    }
