var imagemagick = function(childproc) {
  var EventEmitter = require('events').EventEmitter,
      util = require('util'),
      identify, readMetadata, convert, resize, crop, resizeArgs;

  if (childproc === undefined) childproc = require('child_process');
  ...
  function exec2(file, args /*, options, callback */) {
    ...
    var NullAccumulator = function() {
      this.out = this.err = this.finish = this.errCurrent = function() { return ''; };
    };

    var ProcStream = function(proc) {
      this.write = proc.stdin.write.bind(proc.stdin);
      this.end = proc.stdin.end.bind(proc.stdin);
      proc.stdout.on('data', this.emit.bind(this, 'data'));
      proc.stdout.on('end', this.emit.bind(this, 'end'));
    };
    util.inherits(ProcStream, EventEmitter);

    var stream = new ProcStream(child);
    var std = callback ? new Accumulator(callback) : new NullAccumulator();
    if (options.output) {
      stream.on('data', options.output.emit.bind(options.output, 'data'));
      stream.on('end', options.output.emit.bind(options.output, 'end'));
    }
  ...
  identify = function(pathOrArgs, callback) {
    ...
    var proc = exec2(identify.path, args, {timeout:120000}, function(err, stdout, stderr) {
      var result, geometry;
      if (!err) {
        if (isCustom) {
          result = stdout;
    ...
    if (isData) {
      if ('string' === typeof pathOrArgs.data) {
        proc.write(pathOrArgs.data, 'binary');
        proc.end();
      } else {
        proc.end(pathOrArgs.data);
      }
    }
    return proc;
  }
  ...
  readMetadata = function() {
    var args = Array.prototype.slice.call(arguments),
        callback = args.pop();
    return identify.apply(undefined, args.concat([function(err, features) {
      var meta = {};
      if (!err) {
        for(prop in features.Properties) {
          var key = prop.replace('/','-'),
              value = features.Properties[prop],
              typekey = 'default';
          var p = key.indexOf(':');
          if (p !== -1) {
  ...
  convert = function(args, procopts, callback) {
    ...
    return exec2(convert.path, args, procopts, callback);
  }
  convert.path = 'convert';

  var resizeCall = function(t, callback) {
    var proc = convert(t.args, {timeout: t.opt.timeout, output: t.opt.dstStream}, callback);
    if (t.opt.srcPath.match(/-$/)) {
      if ('string' === typeof t.opt.srcData) {
        proc.write(t.opt.srcData, 'binary');
        proc.end();
      } else {
        proc.end(t.opt.srcData);
      }
    }
    return proc;
  }
  ...
  resize = function(options, callback) {
    var t = resizeArgs(options);
    return resizeCall(t, callback)
  }

  crop = function (options, callback) {
    var stream = options.dstStream || new EventEmitter();

    if (typeof options !== 'object')
      throw new TypeError('First argument must be an object');
    if (!options.srcPath && !options.srcData)
      throw new TypeError("No srcPath or srcData defined");
    if (!options.height && !options.width)
      throw new TypeError("No width or height defined");

    identify(options.srcPath || {data: options.srcData}, function(err, meta) {
